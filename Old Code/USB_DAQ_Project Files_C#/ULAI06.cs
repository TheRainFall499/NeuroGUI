// ==============================================================================
// Barebones DAQ Software from modified ULAI06.cs example program
//
...
namespace ULAI06
{
	public class frmStatusDisplay : System.Windows.Forms.Form
	{
        ...
        private bool wrote1 = false;
        private bool wrote2 = false;

        FileStream fs = File.Create("C:\\Users\\Michael\\Documents\\binary.dat", 2048, FileOptions.None);
        
        //const int NumPoints = 31746;  //  Number of data points to collect
        const int NumPoints = 600000;
        ...
        private void tmrCheckStatus_Tick(object eventSender, System.EventArgs eventArgs)
        {
            ...
            if (CurIndex > (NumPoints / 2)+500) //If the current index is in the second half of the buffer, write the first half to an array.
            {
                if (ADResolution > 16)
                {
                    /* We don't have the code for this part yet.
                    ...
                    for (i = 0; i <= HighChan; ++i)
                    {
                        ...
                        string spitout = ADData32[i].ToString("G");
                        lblADData[i].Text = "working";
                ...
                {
                    /*if (secondwrite == false)
                    {
                        secondwrite = true;
                        Debug.WriteLine("SecondWrite is True");
                    }*/
                    if (firstrun != false)
                        firstrun = false;
                    if (wrote2 != false)
                    ...
                    if (wrote1 == false)
                    {
                        //Fill ADData with the first half of the memory buffer.
                        ULStat = MccDaq.MccService.WinBufToArray(MemHandle, ADData, 0, NumPoints);
                        using (var bw = new BinaryWriter(fs, Encoding.BigEndianUnicode, true))
                        {
                            Debug.WriteLine("FirstHalfStart = 0");
                            for (i = 0; i < (NumPoints / 2); ++i)
                            {
                                float voltvalue;
                                DaqBoard.ToEngUnits(Range, ADData[i], out voltvalue);
                                bw.Write(voltvalue);
                                if (i > (NumPoints/2)-3)
                                Debug.WriteLine("FirstHalfEnd = " + i.ToString());
                            }
            ...
            if (CurIndex < (NumPoints/2)+500) //write second half.
            {
                if (wrote1 != false)
                    wrote1 = false;
                if (firstrun == true)
                    wrote2 = true;
                if (wrote2 == false)  //If currentindex is < (NumPoints/2) only write second half when it exists... after the frist buffer fill.
                {
                    ULStat = MccDaq.MccService.WinBufToArray(MemHandle, ADData, 0, NumPoints);
                    using (var bw = new BinaryWriter(fs, Encoding.BigEndianUnicode, true))
                    {
                        Debug.WriteLine("SecondHalfStart = " + (NumPoints / 2).ToString());
                        for (i = (NumPoints/2); i < NumPoints; ++i)
                        {
                            ...
                            bw.Write(voltvalue);
                            if (i > NumPoints-3 )
                            Debug.WriteLine("SecondHalfEnd = " + i.ToString());
                        }
		...
		private void InitializeComponent()
		{
            ...
            this.lblShowIndex.Click += new System.EventHandler(this.lblShowIndex_Click);
            // 
            // lblIndex
            // 
            ...
            this._lblADData_0.Click += new System.EventHandler(this._lblADData_0_Click);
            // 
            // lblChan0
            // 
            ...
            this.lblDemoFunction.Text = "Test of Mccdaq.MccBoard.AInScan() in Continuous Background mode";
        ...
        private void lblDemoFunction_Click(object sender, EventArgs e)
        {
        ...
        private void lblShowIndex_Click(object sender, EventArgs e)
        {

        }

        private void _lblADData_0_Click(object sender, EventArgs e)
        {
